...
# This function checks if the filter file exists, if it meets protocol's type and, it writes its content to the configuration file.
prepare_bgp_filters() {
    local section="$1"
    local type
    local file_path
...
# Function: prepare_ospf_filters $1
# $1 string
# This function gets each "ospf_filter" protocol section in the UCI configuration and sets each option in the bird4.conf file.
# $1 is set as the ID of the current UCI ospf_filter section.
# This function checks if the filter file exists, if it meets protocol's type and, it writes its content to the configuration file.
prepare_ospf_filters() {
    local section="$1"
    local type
    local file_path
    get type $section
